...
function tmpFile(): string {
  return join(
    tmpdir(),
    `anima-test-obs-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
  );
}
...
describe('ObservationEvaluator.handler()', () => {
  ...
  it('stores a task_result memory after a successful run', async () => {
    ...
    await ev.handler(
      runtime,
      makeMessage('Analyse the codebase'),
      makeContent('The codebase uses TypeScript with Nx.'),
    );
  ...
  it("tags the task_result with the agent's name", async () => {
    ...
    await ev.handler(
      runtime,
      makeMessage('Write a summary'),
      makeContent('Summary written.'),
    );

    const all = manager.getRecent({ agentName: 'writer' });
    const writerTask = all.find(
      (m) => m.agentName === 'writer' && m.type === 'task_result',
    );
  ...
  it('stores an observation when result is longer than 100 chars', async () => {
    const runtime = makeRuntime();
    const longResult =
      'The Nx workspace contains multiple packages organized by domain. Each package has its own build targets.';
    expect(longResult.length).toBeGreaterThan(100);

    await ev.handler(
      runtime,
      makeMessage('Describe the workspace'),
      makeContent(longResult),
    );
  ...
  it('does NOT store an observation when result is 100 chars or fewer', async () => {
    ...
    await ev.handler(
      runtime,
      makeMessage('Short task'),
      makeContent(shortResult),
    );
  ...
  it('observation content is the first sentence of the result', async () => {
    ...
    const result =
      'The system uses BM25 for search ranking and relevance scoring. It also has persistence via JSON files. And even more features beyond that.';
    expect(result.length).toBeGreaterThan(100);
    await ev.handler(
      runtime,
      makeMessage('Describe the system'),
      makeContent(result),
    );
    ...
    expect(observations[0].content).toContain(
      'The system uses BM25 for search ranking and relevance scoring',
    );
    ...
    expect(observations[0].content).not.toContain(
      'And even more features beyond that',
    );
  });
  ...
  it('truncates very long tasks to 120 chars in the task_result content', async () => {
    ...
    await ev.handler(
      runtime,
      makeMessage(longTask),
      makeContent('Done with the long task.'),
    );
  ...
  it('multiple handler calls accumulate memories independently', async () => {
    ...
    await ev.handler(
      runtime,
      makeMessage('Task one'),
      makeContent('Result one.'),
    );
    await ev.handler(
      runtime,
      makeMessage('Task two'),
      makeContent('Result two.'),
    );
    await ev.handler(
      runtime,
      makeMessage('Task three'),
      makeContent('Result three.'),
    );

    const taskResults = manager
      .getRecent()
...
describe('ObservationEvaluator.handler() persistence', () => {
  ...
  it('persists memories to disk so a reloaded manager can access them', async () => {
    ...
    await ev.handler(
      ...
      makeContent(
        'Nx is a powerful build system for monorepos with smart caching.',
      ),
    );
