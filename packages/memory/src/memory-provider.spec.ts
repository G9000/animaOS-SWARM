...
describe('MemoryProvider.get()', () => {
  ...
  it('returns empty text when the memory store is empty', async () => {
    ...
    const result = await provider.get(
      runtime,
      makeMessage('what do we know about TypeScript'),
    );
  ...
  it("formats each memory as '- [type] content (age)'", async () => {
    ...
    expect(result.text).toMatch(
      /- \[fact\] BM25 is a ranking algorithm \(\d+m ago\)/,
    );
  });
  ...
  it("scopes search to the agent's own name", async () => {
    ...
    manager.add({
      ...
      agentName: 'researcher',
      type: 'fact',
      content: 'researcher knows TypeScript deeply',
      importance: 0.9,
    });
    manager.add({
      agentId: 'a2',
      agentName: 'writer',
      type: 'fact',
      content: 'writer knows TypeScript basics',
      importance: 0.9,
    });
  ...
  it('uses agentName (not agentId) for scoping so memories persist across runs', async () => {
    ...
    manager.add({
      ...
      agentName: 'researcher',
      type: 'fact',
      content: 'Nx is a monorepo build system',
      importance: 0.8,
    });
  ...
  it('includes recent memories via getRecent() even when unrelated to the query', async () => {
    ...
    const result = await provider.get(
      runtime,
      makeMessage('quantum physics neutron stars'),
    );
  ...
  it('deduplicates memories that appear in both search results and recent', async () => {
    ...
    manager.add({
      ...
      agentName: 'researcher',
      type: 'fact',
      content: 'TypeScript has generics',
      importance: 0.9,
    });
    manager.add({
      agentId: 'a1',
      agentName: 'researcher',
      type: 'fact',
      content: 'TypeScript has interfaces',
      importance: 0.9,
    });

    const result = await provider.get(
      runtime,
      makeMessage('TypeScript generics interfaces'),
    );
    // Count occurrences — a deduplicated result won't repeat the same content
    const occurrences = (result.text.match(/TypeScript has generics/g) ?? [])
      .length;
  ...
  it('returns metadata.memoryCount equal to the exact number of memories included', async () => {
    ...
    manager.add({
      ...
      agentName: 'researcher',
      type: 'observation',
      content: 'Vitest integrates with Vite',
      importance: 0.7,
    });
  ...
  it('search() excludes memories below minImportance 0.2; getRecent() may still include them', async () => {
    ...
    for (let i = 1; i <= 5; i++) {
      manager.add({
        ...
        agentName: 'researcher',
        type: 'fact',
        content: `high importance fact ${i}`,
        importance: 0.9,
      });
    }
    ...
    manager.add({
      ...
      agentName: 'researcher',
      type: 'fact',
      content: 'barely relevant obscure widget detail',
      importance: 0.1,
    });
    ...
    const result = await provider.get(
      runtime,
      makeMessage('high importance fact'),
    );
