...
function baseConfig(strategy: SwarmConfig["strategy"], workerNames: string[]): SwarmConfig {
	return {
		strategy,
...
describe("SwarmCoordinator basics", () => {
	it("has a unique id on construction", () => {
		...
		const a = new SwarmCoordinator(baseConfig("round-robin", ["worker"]), adapter)
		const b = new SwarmCoordinator(baseConfig("round-robin", ["worker"]), adapter)
	...
	it("starts in idle status", () => {
		const coord = new SwarmCoordinator(baseConfig("round-robin", ["worker"]), textAdapter("hi"))
	...
	it("emits swarm:created event on run", async () => {
		...
		const coord = new SwarmCoordinator(baseConfig("round-robin", ["worker"]), textAdapter("hi"), bus)
		await coord.run("emit test")

		expect(listener).toHaveBeenCalledOnce()
		// EventBus wraps payload in Event<T>: { type, timestamp, agentId?, data }
		expect(listener.mock.calls[0][0].data).toMatchObject({ strategy: "round-robin" })
	})
	...
	it("returns error result when the model adapter fails", async () => {
		...
		const coord = new SwarmCoordinator(baseConfig("round-robin", ["worker"]), brokenAdapter)
	...
	it("result has a durationMs >= 0", async () => {
		const coord = new SwarmCoordinator(baseConfig("round-robin", ["worker"]), textAdapter("done"))
	...
	it("throws for unknown strategy — getStrategy() throws before the try-catch", async () => {
		...
		const coord = new SwarmCoordinator(
			{ strategy: "unknown" as SwarmConfig["strategy"], manager: managerConfig(), workers: [] },
			textAdapter("hi"),
		)
...
describe("round-robin strategy", () => {
	it("returns success status", async () => {
		...
		const coord = new SwarmCoordinator(baseConfig("round-robin", ["w1", "w2"]), adapter)
	...
	it("result data includes history array with speaker/content pairs", async () => {
		...
		const data = result.data as { history: Array<{ speaker: string; content: string }> }
	...
	it("first turn passes the raw task to the first agent", async () => {
		...
		const options = firstCall[1] as { messages: Array<{ content: { text: string } }> }
		const hasTask = options.messages.some((m) => m.content?.text?.includes("The actual task text"))
...
describe("supervisor strategy", () => {
	it("returns success after manager synthesizes worker results", async () => {
		...
		const adapter = sequenceAdapter([
			{
				content: { text: "" },
				toolCalls: [{ id: "tc1", name: "delegate_task", args: { worker_name: "worker", task: "Do research" } }],
				usage: { promptTokens: 10, completionTokens: 5, totalTokens: 15 },
		...
		const coord = new SwarmCoordinator(baseConfig("supervisor", ["worker"]), adapter)
	...
	it("returns error when delegate_task names an unknown worker", async () => {
		...
		const adapter = sequenceAdapter([
			{
				content: { text: "" },
				toolCalls: [{ id: "tc1", name: "delegate_task", args: { worker_name: "nonexistent", task: "Do it" } }],
				usage: { promptTokens: 10, completionTokens: 5, totalTokens: 15 },
				stopReason: "tool_call",
			},
			{
	...
	it("manager can delegate to a worker and the worker's result feeds back into the manager", async () => {
		...
		const adapter = sequenceAdapter([
			{
				content: { text: "" },
				toolCalls: [{ id: "tc1", name: "delegate_task", args: { worker_name: "worker", task: "Do work" } }],
				usage: { promptTokens: 10, completionTokens: 5, totalTokens: 15 },
				stopReason: "tool_call",
			},
			{
	...
	it("records spawned agent IDs in state", async () => {
		...
		const coord = new SwarmCoordinator(baseConfig("supervisor", ["w1", "w2"]), adapter)
...
describe("dynamic strategy", () => {
	it("returns success when manager signals DONE", async () => {
		...
		const adapter = sequenceAdapter([
			{
				content: { text: "" },
				toolCalls: [{ id: "tc1", name: "choose_speaker", args: { agent_name: "DONE", instruction: "wrap up" } }],
				usage: { promptTokens: 10, completionTokens: 5, totalTokens: 15 },
				stopReason: "tool_call",
			},
			{
		...
		const coord = new SwarmCoordinator(baseConfig("dynamic", ["analyst"]), adapter)
	...
	it("returns error when choose_speaker names an unknown agent", async () => {
		const adapter = sequenceAdapter([
			{
				content: { text: "" },
				toolCalls: [{ id: "tc1", name: "choose_speaker", args: { agent_name: "ghost" } }],
				usage: { promptTokens: 10, completionTokens: 5, totalTokens: 15 },
				stopReason: "tool_call",
			},
			{
	...
	it("records all spawned agent IDs including workers and manager", async () => {
		...
		const coord = new SwarmCoordinator(baseConfig("dynamic", ["a1", "a2"]), adapter)
	...
	it("passes chat history to chosen speaker as context", async () => {
		...
		const generate = vi.fn()
			.mockResolvedValueOnce({
				...
				content: { text: "" },
				toolCalls: [{ id: "tc1", name: "choose_speaker", args: { agent_name: "analyst", instruction: "analyse data" } }],
				usage: { promptTokens: 10, completionTokens: 5, totalTokens: 15 },
				stopReason: "tool_call",
			} satisfies GenerateResult)
			.mockResolvedValueOnce({
			...
			.mockResolvedValueOnce({
				// Manager: DONE
				content: { text: "" },
				toolCalls: [{ id: "tc2", name: "choose_speaker", args: { agent_name: "DONE" } }],
...
describe("SwarmCoordinator.getState()", () => {
	it("agentIds is empty before any run", () => {
		const coord = new SwarmCoordinator(baseConfig("round-robin", ["w1"]), textAdapter("hi"))
		expect(coord.getState().agentIds).toHaveLength(0)
	})

	it("results array contains the task result after run", async () => {
		const coord = new SwarmCoordinator(baseConfig("round-robin", ["w1"]), textAdapter("final"))
	...
	it("tokenUsage is non-zero after a run", async () => {
		...
		const coord = new SwarmCoordinator(baseConfig("round-robin", ["w1"]), textAdapter("ok"))
