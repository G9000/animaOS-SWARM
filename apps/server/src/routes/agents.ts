...
export const agentRoutes = [
	...
	route("GET", "/api/agents", async (_req, res, state) => {
		const agents = Array.from(state.agents.values()).map((a) => {
			const s = a.getState()
			return { id: s.id, name: s.name, status: s.status, tokenUsage: s.tokenUsage }
		})
		json(res, 200, { agents })
	}),
	...
	// Run task on agent
	route("POST", "/api/agents/:id/run", async (_req, res, state, body, params) => {
	...
	// Delete agent
	route("DELETE", "/api/agents/:id", async (_req, res, state, _body, params) => {
