...
async fn create_memory_entry(State(state): State<AppState>, request: Request) -> AxumResponse {
    match read_limited_body(request, state.config.max_request_bytes).await {
        Ok(body) => match memories::handle_create_memory(body, &state.daemon) {
...
async fn memories_search_entry(State(state): State<AppState>, uri: Uri) -> AxumResponse {
    handle_memory_search(uri, &state.daemon)
}
...
async fn memories_recent_entry(State(state): State<AppState>, uri: Uri) -> AxumResponse {
    ...
    match memories::handle_recent_memories(query, &state.daemon) {
...
async fn list_agents_entry(State(state): State<AppState>) -> AxumResponse {
    match agents::handle_list_agents(&state.daemon) {
        Ok(response) => json_response(StatusCode::OK, &response),
        Err(error) => error.into_response(),
    }
}
...
async fn create_agent_entry(State(state): State<AppState>, request: Request) -> AxumResponse {
    match read_limited_body(request, state.config.max_request_bytes).await {
        Ok(body) => match agents::handle_create_agent(body, &state.daemon) {
            Ok(response) => json_response(StatusCode::CREATED, &response),
            Err(error) => error.into_response(),
        },
        Err(response) => response,
...
async fn get_agent_entry(
    ...
    match agents::handle_get_agent(&agent_id, &state.daemon) {
        Ok(response) => json_response(StatusCode::OK, &response),
        Err(error) => error.into_response(),
    }
}
...
async fn delete_agent_entry(
    ...
    match agents::handle_delete_agent(&agent_id, &state.daemon) {
        Ok(response) => json_response(StatusCode::OK, &response),
        Err(error) => error.into_response(),
    }
}
...
async fn agent_recent_memories_entry(
    ...
    match agents::handle_recent_agent_memories(&agent_id, query, &state.daemon) {
        Ok(response) => json_response(StatusCode::OK, &response),
        Err(error) => error.into_response(),
    }
}
...
async fn list_swarms_entry(State(state): State<AppState>) -> AxumResponse {
    match swarms::handle_list_swarms(&state.daemon) {
        Ok(response) => json_response(StatusCode::OK, &response),
        Err(error) => error.into_response(),
    }
}
...
async fn create_swarm_entry(State(state): State<AppState>, request: Request) -> AxumResponse {
    match read_limited_body(request, state.config.max_request_bytes).await {
...
async fn get_swarm_entry(
    ...
    match swarms::handle_get_swarm(&swarm_id, &state.daemon) {
        Ok(response) => json_response(StatusCode::OK, &response),
        Err(error) => error.into_response(),
    }
}
...
async fn swarm_events_entry(
    ...
    swarms::handle_subscribe_swarm_events(&swarm_id, &state.daemon)
}
...
fn handle_memory_search(uri: Uri, state: &SharedDaemonState) -> AxumResponse {
    let query = match request_query(&uri) {
    ...
    match memories::handle_search_memories(query, state) {
        Ok(response) => json_response(StatusCode::OK, &response),
        Err(error) => error.into_response(),
    }
}
