...
| `ANIMAOS_RS_SESSION_EVENT_BUFFER`                     | No                         | Events buffered per agent for the live event stream (`GET /api/agents/{agent_id}/events`) before a slow stream lags (default `1024`, rounded up to a power of two). A stream that falls behind receives `stream.resync` and keeps going. `0` or a non-numeric value fails startup with `ANIMAOS_RS_SESSION_EVENT_BUFFER must be a positive integer`.                                                                                                                                                                                                                                                                                                                                                           |
