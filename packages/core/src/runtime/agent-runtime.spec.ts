...
describe('AgentRuntime', () => {
  ...
  it('includes tool output in tool:after events', async () => {
    const tool = mockToolAction('search', 'found results');
    const adapter: IModelAdapter = {
      provider: 'test',
      generate: vi
    ...
    const bus = new EventBus();
    const seen: Array<{ result?: string }> = [];
    bus.on<{ result?: string }>('tool:after', (evt) => {
      seen.push(evt.data);
    });
  ...
  it('should throw when sending without coordinator', async () => {
    ...
    await expect(runtime.send('other', { text: 'hi' })).rejects.toThrow(
      'not connected to a swarm coordinator'
    );
  });
});
