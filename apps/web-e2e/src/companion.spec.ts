...
async function fixture(page: Page, empty = false) {
  ...
  const helper = {
    ...
    state: {
      ...
      config: {
        ...agent.state.config,
        settings: { additional: { workspaceRole: 'helper' } },
      },
    },
  };
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (path === '/api/health') body = { status: 'ok' };
      ...
      if (route.request().method() === 'POST') {
        created = true;
        body = { session: chatSession };
      } else
        body = { sessions: created ? [chatSession] : [], nextCursor: null };
    } else if (
      /^\/api\/agents\/companion\/sessions\/[^/]+\/messages$/.test(path)
    ) {
      body = {
        messages: (
          agent.messages as {
    ...
    } else if (/^\/api\/agents\/companion\/sessions\/[^/]+$/.test(path))
      body = { session: chatSession };
      ...
      agent.messages = [
        {
          ...
          role: 'user',
          agentId: 'companion',
          roomId: route.request().postDataJSON().roomId,
          content: { text },
          createdAtMs: 2,
        },
        {
          ...
          roomId: route.request().postDataJSON().roomId,
          content: {
            text: 'Let’s make a little room in your day. What matters most today?',
          },
          createdAtMs: 3,
        },
      ];
...
for (const viewport of [
  ...
  test(`${viewport.name}: companion chat, navigation and settings`, async ({
    page,
  }, testInfo) => {
    ...
    await page.goto('/');
    await expect(
      page.getByRole('heading', { name: 'Say something to Anima' }),
    ).toBeVisible();
    await expect(
    ...
    await page.screenshot({
      path: testInfo.outputPath(`${viewport.name}-welcome.png`),
      fullPage: true,
    });
    ...
    await expect(
      page.getByText(
        'Let’s make a little room in your day. What matters most today?',
      ),
    ).toBeVisible();
    ...
    await page.screenshot({
      path: testInfo.outputPath(`${viewport.name}-conversation.png`),
      fullPage: true,
    });
...
test('first run creates one companion without a team wizard', async ({
  page,
}, testInfo) => {
