...
const ARRAY_FIELDS: EditableField[] = ["adjectives", "topics", "knowledge"]
...
function applyValue(profile: AgentProfile, field: EditableField, value: string): AgentProfile {
...
export function AgentsPanel({ profiles, onBack, onSave }: AgentsPanelProps): React.ReactElement {
  ...
  useInput((input, key) => {
    if (phase === "list") {
      ...
      else if (key.downArrow) setAgentIdx((i) => Math.min(profiles.length - 1, i + 1))
          ...
          {profiles.map((p, i) => {
            ...
            return (
              <Box key={p.name}>
                ...
                <Text color="gray">{"  "}{p.role ?? "worker"}</Text>
                {p.adjectives && p.adjectives.length > 0 ? (
                  <Text color="gray">{"  ·  "}{p.adjectives.slice(0, 3).join(", ")}</Text>
                ) : null}
              </Box>
            )
          })}
    ...
    return (
      <Box flexDirection="column" borderStyle="single" paddingX={1}>
        ...
        {p.bio ? (
          <Box flexDirection="column" marginTop={1}>
            <Text bold>Bio</Text>
            <Text color="gray">{"  "}{p.bio}</Text>
          </Box>
        ) : null}
        {p.lore ? (
          <Box flexDirection="column" marginTop={1}>
            <Text bold>Backstory</Text>
            <Text color="gray">{"  "}{p.lore}</Text>
          </Box>
        ) : null}
        {p.adjectives && p.adjectives.length > 0 ? (
          <Box marginTop={1}>
        ...
        {p.style ? (
          <Box flexDirection="column" marginTop={1}>
            ...
            <Text color="gray">{"  "}{p.style}</Text>
          </Box>
        ) : null}
        {p.system ? (
          <Box flexDirection="column" marginTop={1}>
            <Text bold>System</Text>
            <Text color="gray">{"  "}{p.system}</Text>
          </Box>
        ) : null}
        <Box marginTop={1}>
          <Text color="gray" dimColor>e edit{"  "}q back to list</Text>
        </Box>
      </Box>
    )
  }
  ...
  return (
    <Box flexDirection="column" borderStyle="single" paddingX={1}>
      <Box>
        ...
        {savedMsg ? <Text color="green">{"  "}✓ saved to anima.yaml</Text> : null}
      </Box>
      <Box flexDirection="column" marginTop={1}>
        {EDITABLE_FIELDS.map((field, i) => {
          const active = i === fieldIdx
          const isArray = ARRAY_FIELDS.includes(field)
          const displayVal = active ? editValue : (draft ? getValue(draft, field) : "")
          return (
            <Box key={field}>
              <Text color={active ? "cyan" : "gray"} bold={active}>
                {active ? "❯ " : "  "}
              ...
              {active && isArray ? (
                <Text color="gray" dimColor>{"  "}comma-separated</Text>
              ) : null}
            </Box>
          )
        })}
