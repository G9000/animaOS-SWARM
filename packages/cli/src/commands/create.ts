...
export const createCommand = new Command("create")
	...
	.action(async (nameArg: string | undefined, opts: { provider: string; model: string; apiKey?: string }) => {
		clack.intro(pc.bgCyan(pc.black(" animaOS-SWARM ")))

		// 1. Agency name
		const name = nameArg ?? await clack.text({
			message: "Agency name:",
		...
		clack.log.message(`  ${pc.yellow("★")} ${pc.cyan(orchestrator.name)} ${pc.dim("(orchestrator)")} — ${orchestrator.bio}`)
		for (const agent of workers) {
			clack.log.message(`  ${pc.dim("•")} ${pc.cyan(agent.name)} — ${agent.bio}`)
		}

		const accepted = await clack.confirm({ message: "Accept this team?" })
		if (clack.isCancel(accepted) || !accepted) { clack.cancel("Cancelled."); process.exit(0) }
		...
		writeFileSync(join(dirPath, "anima.yaml"), yaml.dump(config, { lineWidth: 120, noRefs: true }))

		clack.log.success(`Agency saved to ${pc.green(dirName + "/anima.yaml")}`)
		clack.outro(`Launch with: ${pc.cyan(`cd ${dirName} && animaos launch "your task"`)}`)
	})
