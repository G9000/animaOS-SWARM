...
const OUTCOME_STYLE: Record<NonNullable<Checkin['lastOutcome']>, { dot: string; label: string }> = {
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="animate-rise-in mx-auto w-full max-w-2xl px-6 py-8">
          <p className="text-sm leading-relaxed text-ink-2">
            ...
            something to say. Each proactive prompt runs on a schedule — a
            reply of exactly <code className="rounded bg-white/5 px-1.5 py-0.5 font-mono text-[11px] text-sky-300">CHECKIN_OK</code> keeps
            the run silent.
          </p>
          ...
          <div className="mt-6 space-y-2">
            {checkins.length === 0 ? (
              <div className="flex flex-col items-center rounded-2xl border border-dashed border-line px-6 py-12 text-center">
                ...
                <p className="text-sm font-medium text-ink-2">Nothing scheduled yet</p>
                <p className="mt-1 text-xs text-ink-3">
                  Add a proactive prompt above and your agent will reach out on its own.
                </p>
              </div>
            ) : (
              checkins.map((c) => (
                ...
                >
                  <div className="flex items-center gap-3.5">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-sm text-ink" title={c.prompt}>
                        {c.prompt}
                      </div>
                  ...
                  {c.lastOutcome === 'spoke' && c.lastReply && (
                    <div className="mt-2.5 truncate rounded-lg border border-line bg-white/[0.02] px-3 py-2 font-mono text-[11px] text-ink-3" title={c.lastReply}>
                      ↳ {c.lastReply}
                    </div>
                  )}
                </div>
