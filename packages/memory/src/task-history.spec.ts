...
describe("TaskHistory", () => {
	function makeEntry(id: string, task: string, result: string, agentId = "agent-1") {
		return {
			id,
			agentId,
			task,
	...
	it("should record and search entries", () => {
		...
		history.record(makeEntry("1", "Research AI agents", "Found 10 papers on multi-agent systems"))
		history.record(makeEntry("2", "Write code for API", "Created REST endpoints"))
		history.record(makeEntry("3", "Deploy to production", "Deployed successfully"))
