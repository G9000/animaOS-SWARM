...
function renderAgentSummary(agent: AgentSnapshot) {
	console.log()
	console.log(`  ${pc.cyan(pc.bold(agent.state.name))} ${pc.dim(`(${agent.state.id})`)}`)
	console.log(`    ${pc.dim("status:")} ${agent.state.status}`)
	console.log(`    ${pc.dim("model:")} ${agent.state.config.model}`)
	console.log(`    ${pc.dim("messages:")} ${agent.messageCount} ${pc.dim("events:")} ${agent.eventCount}`)
}

function renderAgentDetails(agent: AgentSnapshot) {
	console.log()
	console.log(pc.bold(pc.cyan(`  ${agent.state.name}`)) + pc.dim(` (${agent.state.id})`))
	console.log()
...
export async function executeAgentsShowCommand(
	...
	if (!match) {
		const knownAgents = agents.map((agent) => `${agent.state.name} (${agent.state.id})`).join(", ")
		console.error(`Error: Agent "${nameOrId}" not found. Available: ${knownAgents}`)
		process.exitCode = 1
		return
	}
