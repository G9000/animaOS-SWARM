...
pub(crate) fn workspace_root_path(tool_name: &str) -> Result<PathBuf, String> {
    match std::env::var("ANIMAOS_WORKSPACE_ROOT") {
        ...
        Err(std::env::VarError::NotPresent) => std::env::current_dir().map_err(|error| {
            format!("{tool_name} workspace root could not be determined: {error}")
        }),
        Err(error) => Err(format!(
            "{tool_name} workspace root could not be read: {error}"
        )),
    }
}
...
pub(super) fn ensure_path_within_workspace(
    ...
    if candidate.starts_with(workspace_root) {
        ...
        Err(format!(
            "{tool_name} path escapes workspace root: {user_path}"
        ))
    }
}
...
pub(crate) fn normalized_relative_path(base_root: &Path, path: &Path) -> Result<String, String> {
    let relative = path
