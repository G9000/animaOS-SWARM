...
#[utoipa::path(
    ...
    path = "/api/workspace",
    tag = "workspace",
    request_body = WorkspaceConfigRequest,
    responses(
        (status = 200, description = "Workspace configuration validated (validate-only) or persisted", body = WorkspaceResponse),
        (status = 400, description = "Invalid request or workspace root path", body = ErrorBody),
        (status = 503, description = "Workspace configuration could not be persisted", body = ErrorBody)
    )
)]
async fn put_workspace_entry(State(state): State<AppState>, request: AxumRequest) -> AxumResponse {
    match read_limited_body(request, state.config.max_request_bytes).await {
        Ok(body) => {
            let _transaction = state.agent_runs.control_plane_transaction().await;
